...
<#
.SYNOPSIS
Tests AzureFirewallPolicyDraftCRUD.
#>
function Test-AzureFirewallPolicyDraftCRUD {
    # Setup
    $rgname = Get-ResourceGroupName
    $azureFirewallPolicyName = Get-ResourceName
    $azureFirewallPolicyAsJobName = Get-ResourceName
    ...
    try {
        ...
        $azureFirewallPolicyDraft = New-AzFirewallPolicyDraft -PolicyName $azureFirewallPolicyName -ResourceGroupName $rgname 
        ...
        $azureFirewallPolicyDraft = New-AzFirewallPolicyDraft -AzureFirewallPolicy $getAzureFirewallPolicy
        $getAzureFirewallPolicyDraft = Get-AzFirewallPolicyDraft -PolicyName $azureFirewallPolicyName -ResourceGroupName $rgName


        # Check rule groups count
        Assert-AreEqual 1 @($getAzureFirewallPolicy.RuleCollectionGroups).Count

        # Check rule groups count draft
        Assert-AreEqual 1 @($getAzureFirewallPolicyDraft.RuleCollectionGroups).Count

        $getRg = Get-AzFirewallPolicyRuleCollectionGroup -Name $ruleGroupName -AzureFirewallPolicy $getAzureFirewallPolicy

        $getRcgDraft = Get-AzFirewallPolicyRuleCollectionGroupDraft -AzureFirewallPolicy $getAzureFirewallPolicy


        Assert-AreEqual 3 @($getRg.properties.ruleCollection).Count

        Assert-AreEqual 3 @(getRcgDraft.properties.ruleCollection).Count


        $filterRuleCollection1 = $getRg.Properties.GetRuleCollectionByName($appRcName)
        $filterRuleCollectionDraft1 = $getRcgDraft.Properties.GetRuleCollectionByName($appRcName)
        $filterRuleCollection2 = $getRg.Properties.GetRuleCollectionByName($networkRcName)
        $natRuleCollection = $getRg.Properties.GetRuleCollectionByName($natRcName)
        ...
        Assert-AreEqual $appRcName $filterRuleCollectionDraft1.Name
        ...
        $testPipelineRgDraft = Get-AzFirewallPolicyRuleCollectionGroupDraft -AzureFirewallPolicyName $getAzureFirewallPolicy.Name -ResourceGroupName $rgname
        $testPipelineRgDraft|Set-AzFirewallPolicyRuleCollectionGroupDraft -Priority $pipelineRcPriorityDraft
        $testPipelineRgDraft = Get-AzFirewallPolicyRuleCollectionGroupDraft -AzureFirewallPolicyName $getAzureFirewallPolicy.Name -ResourceGroupName $rgname
        Assert-AreEqual $pipelineRcPriorityDraft $testPipelineRgDraft.properties.Priority

        Deploy-AzureFirewallPolicyDraft -AzureFirewallPolicyName $getAzureFirewallPolicy.Name -ResourceGroupName $rgname

        $testPipelineRg = Get-AzFirewallPolicyRuleCollectionGroup -Name $ruleGroupName -AzureFirewallPolicyName $getAzureFirewallPolicy.Name -ResourceGroupName $rgname
        Assert-AreEqual $pipelineRcPriorityDraft $testPipelineRg.properties.Priority
