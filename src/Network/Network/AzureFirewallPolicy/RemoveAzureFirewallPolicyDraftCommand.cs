...
namespace Microsoft.Azure.Commands.Network
{
    ...
    public class RemoveAzureFirewallPolicyDraftCommand : AzureFirewallPolicyBaseCmdlet
    {
        ...
        [Parameter(
            ...
            HelpMessage = "The policy name.", ParameterSetName = RemoveByNameParameterSet)]
        ...
        public override void Execute()
        {
            if (this.IsParameterBound(c => c.ResourceId))
            {
                ...
                this.PolicyName = resourceInfo.ResourceName;
            }
            else if (this.IsParameterBound(c => c.AzureFirewallPolicy))
            {
                this.ResourceGroupName = AzureFirewallPolicy.ResourceGroupName;
                this.PolicyName = AzureFirewallPolicy.Name;
            }

            base.Execute();
            ConfirmAction(
                Force.IsPresent,
                string.Format(Properties.Resources.RemovingResource, PolicyName),
                Properties.Resources.RemoveResourceMessage,
                PolicyName,
                () =>
                {
                    this.AzureFirewallPolicyClient.DeleteDraft(this.ResourceGroupName, this.PolicyName);
                    if (PassThru)
                    {
                        WriteObject(true);
                    }
